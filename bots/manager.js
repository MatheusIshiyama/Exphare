...
bot.once("ready", () => {
    ...
    bot.user.setPresence({
        activity: {
            name: "lo-fi | precisa de ajuda? digite !comandos",
            type: 2,
        },
    });
    bot.channels.cache
        .get("781681804008357888")
        .messages.fetch()
        .then((msg) => msg.forEach((msg) => msg.delete()));
    bot.channels.cache
        .get("782269834750197770")
        .messages.fetch()
        .then((msg) => msg.forEach((msg) => msg.delete()));
});
...
bot.on("voiceStateUpdate", async (oldState, newState) => {
    ...
    botsIds.forEach((botId) => {
        if (newState.id === botId) isBot = true;
    });
    if (isBot) return;
    role(oldState);
...
async function botPresence() {
    ...
    if (presence) {
        bot.user.setPresence({
            activity: { name: "Exphare deseja bons estudos a todos!" },
        });
    } else {
        bot.user.setPresence({
            activity: {
